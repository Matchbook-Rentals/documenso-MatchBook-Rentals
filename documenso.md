...
const createTemplateConfig = (presignToken, externalId) => {
  ...
  // Encode config for URL hash (token goes in query params)
...
function TemplateCreator({ presignToken, externalId, onTemplateCreated }) {
  ...
  const iframeUrl = `https://documenso-matchbook-rentals-production.up.railway.app/embed/v1/authoring/template.create?token=${presignToken}#${encodedConfig}`;
